...
const ApplicationConfig = require('./models/applicationConfig.js'); // Adjust path as needed

const mongoURI = 'mongodb://mongo:27017/Policy_Configuration_Service';
...
// Define a function to create the collection and add data
async function createCollectionAndAddData() {
  try {
    ...
    await db.once('open', async () => {
      ...
      if (!(await db.listCollections().toArray()).find(col => col.name === 'applicationconfigs')) {
        await db.createCollection('applicationconfigs');
        console.log('Collection "applicationconfigs" created');
      } else {
        console.log('Collection "applicationconfigs" already exists');
      }

      // Create an object of the ApplicationConfig model
      const newApplicationConfig = new ApplicationConfig({ 
        ...
        "policiesConfiguration": {
          ...
          "defaultPolicyDecision": "permit",
          "bindingAlgorithm": "DENY_UNLESS_PERMIT"
        }
      });

      // Save the object to MongoDB
      await newApplicationConfig.save();
      console.log('ApplicationConfig object saved successfully');
      mongoose.disconnect();
    });
  } catch (error) {
...
// Call the function to create collection and add data
createCollectionAndAddData();